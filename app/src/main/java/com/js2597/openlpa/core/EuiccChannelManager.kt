...
class EuiccChannelManager(private val context: Context) {
    ...
    private val tm by lazy {
        (context.applicationContext as OpenLPAApplication).telephonyManager
    }
    ...
    private suspend fun tryOpenLPAChannel(uiccInfo: UiccCardInfo): EuiccChannel? {
        lock.withLock {
            ensureSEService()
            ...
            if (uiccInfo.isEuicc && !uiccInfo.isRemovable) {
                /* API 29: force use TelephonyManager/OMAPI for built-in eUICC
    ...
    suspend fun enumerateEuiccChannels() {
        withContext(Dispatchers.IO) {
            ...
            for (uiccInfo in tm.uiccCardsInfo) {
                if (tryOpenLPAChannel(uiccInfo) != null) {
